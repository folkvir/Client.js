...
FragmentsClient.prototype.emitError = function (error, stack) {
  var self = this;
  self.events.emit('error', error, stack);
};
...
FragmentsClient.prototype.getFragmentByPattern = function (pattern) {
  ...
  // listen and emit errors on a specific ouput
  fragment.on('error', self.emitError);
...
Fragment.prototype.loadFromUrl = function (pageUrl) {
  ...
  rawPage.on('error', function (error, stack) {
    console.log('Rawpage Error (FragmentsClient.js): ', error, stack);
    fragment.emit('error', error, stack);
