...
var HttpClient = require('../util/HttpClient'),
    ...
    LoggerPlus = require('../util/LoggerPlus.js'),
    _ = require('lodash');
...
function FragmentsClient(startFragment, options) {
  ...
  options = _.defaults(options || { loggerPlus:{ name: 'FragmentsClient' } }, { contentType: DEFAULT_ACCEPT });
  // New LoggerPlus
  this.logger = new LoggerPlus(options.loggerPlus);
  ...
  if (startFragment) {
    ...
    startFragment.once('error', function (error) {
      console.log('StartFragment Error (FragmentsClient.js): ', error);
...
FragmentsClient.prototype.getFragmentByPattern = function (pattern) {
  ...
  function startFragmentError() {
    self.logger.log('error', startFragment.error);
    console.log('StartFragmentError callback (FragmentsClient.js): ', startFragment.error);
    fragment.emit('error', startFragment.error);
    fragment.close();
  }
...
Fragment.prototype.loadFromUrl = function (pageUrl) {
  ...
  rawPage.on('error', function (error, request) {
    console.log('Rawpage Error (FragmentsClient.js): ', error, request);
    fragmentsClient.logger.log('error', error, request);
    fragment.emit('error', error, request);
  });
  ...
  rawPage.getProperty('statusCode', function (statusCode) {
    fragmentsClient.logger.log('info', 'Code: ' + statusCode + ' , Url: ' + pageUrl);
    if (statusCode !== 200) {
      fragmentsClient.logger.log('error', new Error('Could not retrieve ' + pageUrl + ' (' + statusCode + ')'));
      rawPage.emit('error', new Error('Could not retrieve ' + pageUrl + ' (' + statusCode + ')'), rawPage);
    ...
    rawPage.getProperty('contentType', function (contentType) {
      ...
      parsedPage.on('error', function (error) {
        fragmentsClient.logger.log('error', error);
