...
HttpClient.prototype._startRequest = function (request) {
  ...
  catch (error) { return setImmediate(handleRequestError, error, httpRequest); }
...
HttpClient.prototype._computeStatistics = function (responseTime, httpResponse) {
  this._emitHttpResponse(httpResponse);
  this._statistics.responseTime.latest = responseTime;
  this._statistics.responseTime.count++;
  this._statistics.responseTime.max = Math.max(this._statistics.responseTime.max || 0, responseTime);
  this._statistics.responseTime.min = Math.min(this._statistics.responseTime.min || responseTime, responseTime);
  this._statistics.responseTime.sum += responseTime;
  this._statistics.responseTime.average = this._statistics.responseTime.sum / this._statistics.responseTime.count;
};

HttpClient.prototype._emitHttpResponse = function (response) {
  this._statistics.events.emit('httpResponse', response);
};
